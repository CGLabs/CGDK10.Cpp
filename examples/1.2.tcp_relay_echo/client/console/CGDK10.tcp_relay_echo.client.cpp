...
void print_title();
void print_endpoint(test_tcp_relay_echo_client* _ptest_tcp_echo_client);
void print_statistics_info(test_tcp_relay_echo_client* _ptest_tcp_echo_client, bool _update_now = false);
void print_setting_info(test_tcp_relay_echo_client* _ptest_tcp_echo_client);
...
int main()
{
	...
	// 4) ...
	print_endpoint(g_ptest_tcp_relay_echo_client);

	// 5) loop
	for (;;)
	{
		// - output traffic info
		print_statistics_info(g_ptest_tcp_relay_echo_client);
		...
		if (_kbhit())
		{
			...
			case	'3':
					...
					break;
			case	'4':
					g_ptest_tcp_relay_echo_client->request_connect(1000);
					break;
			case	'5':
					g_ptest_tcp_relay_echo_client->request_connect(10000);
					break;
			...
			// message
			case	'j':
					g_ptest_tcp_relay_echo_client->increase_message_size();
					break;

			case	'm':
					g_ptest_tcp_relay_echo_client->decrease_message_size();
					break;
			...
			case	ASCII_CODE_ENTER:
					{
						...
						// - set endpoint
						g_ptest_tcp_relay_echo_client->set_endpoint(str_address, str_port);
			...
			case	'=':
					{
						...
						std::cout << "\n";
						print_endpoint(g_ptest_tcp_relay_echo_client);
						print_statistics_info(g_ptest_tcp_relay_echo_client, true);
					}
					break;
			...
			// output setting info
			print_setting_info(g_ptest_tcp_relay_echo_client);
		}
...
void _print_setting_info(shared_buffer& _buffer_output, test_tcp_relay_echo_client* _ptest_tcp_echo_client)
{
	// 1) move cursor
	...
	// 2) ...
	_buffer_output.append_text("\x1b[37m [relay test]  \x1b[0m");
	_buffer_output.append_text("\x1b[90mmessage size \x1b[0m"); _buffer_output.append_text<char>(str_index_to_string[_ptest_tcp_echo_client->m_selected_message_size]);
}

void _print_test_result(shared_buffer& _buffer_output, test_tcp_relay_echo_client* _ptest_tcp_echo_client)
{
	// 1) output relay messages per second
	_buffer_output.append_text("\x1b[37m              \x1b[90mrelay mmessages/sec \x1b[0m"); _buffer_output.append_text<char>(_ptest_tcp_echo_client->m_receive_message_per_sec);

	// 3) move cursor
	_buffer_output.append_text("\r\x1b[5A");
}
...
void print_title()
{
	...
	buf_output.append_text("\n");
	buf_output.append_text("\x1b[97m  CGCII tcp relay echo client \x1b[0m\n");
	buf_output.append_text("\n");
	buf_output.append_text("\n");
...
void print_setting_info(test_tcp_relay_echo_client* _ptest_tcp_echo_client)
{
	...
	_print_test_result(buf_output, _ptest_tcp_echo_client);
...
void print_statistics_info(test_tcp_relay_echo_client* _ptest_tcp_echo_client, bool _update_now)
{
