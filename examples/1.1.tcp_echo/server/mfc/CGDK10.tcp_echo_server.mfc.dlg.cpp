...
Ctcp_echo_server_dlg::Ctcp_echo_server_dlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_TEST_ECHO_SERVERMFC_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void Ctcp_echo_server_dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

#pragma warning(disable:4407)
BEGIN_MESSAGE_MAP(Ctcp_echo_server_dlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDOK, &Ctcp_echo_server_dlg::OnBnClickedOk)
	ON_BN_CLICKED(IDCANCEL, &Ctcp_echo_server_dlg::OnBnClickedCancel)
	ON_BN_CLICKED(IDC_BUTTON_SEND_1, &Ctcp_echo_server_dlg::OnBnClickedButtonSend1)
	ON_BN_CLICKED(IDC_BUTTON_SEND_2, &Ctcp_echo_server_dlg::OnBnClickedButtonSend2)
	ON_BN_CLICKED(IDC_BUTTON_SEND_3, &Ctcp_echo_server_dlg::OnBnClickedButtonSend3)
	ON_BN_CLICKED(IDC_BUTTON_DISCONNECT_ALL, &Ctcp_echo_server_dlg::OnBnClickedButtonDisconnectAll)
	ON_BN_CLICKED(IDC_BUTTON_CLOSESOCKET_SINGLE, &Ctcp_echo_server_dlg::OnBnClickedButtonClosesocketSingle)
	ON_BN_CLICKED(IDC_BUTTON_DISCONNECT_SINGLE, &Ctcp_echo_server_dlg::OnBnClickedButtonDisconnectSingle)
	ON_BN_CLICKED(IDC_BUTTON_OBJECTPOOL_STATUS, &Ctcp_echo_server_dlg::OnBnClickedButtonObjectpoolStatus)
	ON_BN_CLICKED(IDC_BUTTON_THREAD_STATUS, &Ctcp_echo_server_dlg::OnBnClickedButtonThreadStatus)
	ON_BN_CLICKED(IDC_CHECK_DISABLE_ECHO, &Ctcp_echo_server_dlg::OnBnClickedCheckDisableEcho)
	ON_COMMAND(ID_SYSTEM_OPTIONS, &Ctcp_echo_server_dlg::OnSystemOptions)
	ON_COMMAND(ID_SYSTEM_CGDKLICENSEINFO, &Ctcp_echo_server_dlg::OnSystemCGDKLicenseInfo)
	ON_COMMAND(ID_SYSTEM_EXIT, &Ctcp_echo_server_dlg::OnSystemExit)
END_MESSAGE_MAP()
#pragma warning(default:4407)


BOOL Ctcp_echo_server_dlg::OnInitDialog()
{
	//-----------------------------------------------------------------
	// 1. 기본적으로 들어 있는 부분.
	//-----------------------------------------------------------------
...
void Ctcp_echo_server_dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	CDialogEx::OnSysCommand(nID, lParam);
}

void Ctcp_echo_server_dlg::OnPaint()
{
	if (IsIconic())
	{
		CPaintDC dc(this);
...
HCURSOR Ctcp_echo_server_dlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void Ctcp_echo_server_dlg::OnBnClickedOk()
{
}

void Ctcp_echo_server_dlg::OnBnClickedCancel()
{
}

void Ctcp_echo_server_dlg::OnBnClickedButtonSend1()
{
	// 1) Acceptor 임시 복사
...
void Ctcp_echo_server_dlg::OnBnClickedButtonSend2()
{
	// 1) Acceptor 임시 복사
	object_ptr<net::io::connective::acceptor::Ntcp> pacceptor = g_pacceptor_client;
...
void Ctcp_echo_server_dlg::OnBnClickedButtonSend3()
{
	// 1) acceptor 임시 복사
	object_ptr<net::io::connective::acceptor::Ntcp> pacceptor = g_pacceptor_client;
...
void Ctcp_echo_server_dlg::OnBnClickedButtonDisconnectAll()
{
	// 1) acceptor 임시 복사
	object_ptr<net::io::connective::acceptor::Ntcp> pacceptor = g_pacceptor_client;
...
void Ctcp_echo_server_dlg::OnBnClickedButtonClosesocketSingle()
{
...
void Ctcp_echo_server_dlg::OnBnClickedButtonDisconnectSingle()
{
	// 1) acceptor의 socket iterator를 얻는다.
	auto iterator = g_pacceptor_client->alloc_iterator();
...
void Ctcp_echo_server_dlg::OnBnClickedButtonObjectpoolStatus()
{
	m_dlg_object_factory_status.ShowWindow(SW_SHOW);
}

void Ctcp_echo_server_dlg::OnBnClickedButtonThreadStatus()
{
	m_dlg_thread_status.ShowWindow(SW_SHOW);
}

void Ctcp_echo_server_dlg::OnBnClickedCheckDisableEcho()
{
	g_disable_echo = ((CButton*)GetDlgItem(IDC_CHECK_DISABLE_ECHO))->GetCheck() == BST_CHECKED;
}

void Ctcp_echo_server_dlg::OnSystemOptions()
{
...
void Ctcp_echo_server_dlg::OnSystemCGDKLicenseInfo()
{
}

void Ctcp_echo_server_dlg::OnSystemExit()
{
	// 1) confirm
...
void Ctcp_echo_server_dlg::OnOK()
{
	CDialogEx::OnOK();
}

void Ctcp_echo_server_dlg::OnCancel()
{
	CDialogEx::OnCancel();
}

void Ctcp_echo_server_dlg::set_socket_type(int _socket_type)
{
	// check) 
...
void Ctcp_echo_server_dlg::create_acceptor(int _SocketType)
{
	// 1) 기존 Acceptor를 제거한다.
	if(g_pacceptor_client.exist())
	{
