...
void group_socket::on_stopping()
{
	...
	for_each_member([=](socket_peer* _pmember)->bool
		{
...
result_code group_socket::on_member_entering(socket_peer* _pmember, sMESSAGE& /*_msg*/)
{
	// 1) register messageable한다.
	_pmember->register_messageable(this);

	// notify)
	transmit_message_to(g_pservice, sMESSAGE_NOTIFY(eMESSAGE::WINDOWS::NOTIFY_UPDATE, CGNOTIFY_SOCKET_USER_ADD, (uintptr_t)_pmember));

	// return) 
	return eRESULT::SUCCESS;
}

uintptr_t group_socket::on_member_leaving(socket_peer* _pmember, uintptr_t _param)
{
	...
	// notify)
	transmit_message_to(g_pservice, sMESSAGE_NOTIFY(eMESSAGE::WINDOWS::NOTIFY_UPDATE, CGNOTIFY_SOCKET_USER_REMOVE, (uintptr_t)_pmember));
...
{
	// 1) get socket
	auto psocket_server = _msg.get_source<socket_peer>();
	...
	try
	{
		...
		//auto vector_updtable = g_pservice->m_presource_manager->request_updatatable_list(version_required, version_now);
		auto pupdtable_resource = g_pservice->m_presource_manager->get_resource(0);

		if (pupdtable_resource.empty() == false)
		{
			// - update sender 각체를 생성한다.
			auto pupdate_sender = make_own<update::sender>();

			// - 업데이트할 리소스 리스트들을 등록한다.
			pupdate_sender->add(pupdtable_resource);

			// - update_server를 socket에 설정한다.
			psocket_server->set_update_sender(std::move(pupdate_sender));
			//psocket_server->m_version_require = version_required;
			//psocket_server->m_version_now = version_now;
