...
public:
			CSphere();
			CSphere(const SPHERE& p_rSphere);
	virtual	~CSphere();
...
public:
	...
	// 3) Bounding info
			void				CG3DBoundInfo();
			void				SetBoundingInfo(const SPHERE& p_rSphere);
			const SPHERE&		GetBounding() const							{	return m_Bounding;}
			const SPHERE&		GetOriginalBounding() const					{	return m_Bounding_Original;}
...
protected:
	// 1) Original Bounding Info
			CG3D::SPHERE		m_Bounding_Original;

	// 2) Transformed Bounding Info
			CG3D::SPHERE		m_Bounding;
};
