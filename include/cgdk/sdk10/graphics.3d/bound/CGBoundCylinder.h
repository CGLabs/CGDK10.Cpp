...
public:
			CCylinder();
			CCylinder(const CYLINDER& p_rSphere);
	virtual	~CCylinder();
...
public:
	...
	// 3) Bounding info
			void				CG3DBoundInfo();
			void				SetBoundingInfo(const CYLINDER& p_rSphere);
			const CYLINDER&		GetBounding() const							{	return m_Bounding;}
			const CYLINDER&		GetOriginalBounding() const					{	return m_Bounding_Original;}
...
protected:
	// 1) Original Bounding Info
			CG3D::CYLINDER	m_Bounding_Original;

	// 2) Transformed Bounding Info
			CG3D::CYLINDER	m_Bounding;
};
