...
public:
			CSegment();
			CSegment(const SEGMENT& p_rSegment);
	virtual	~CSegment();
...
public:
	...
	// 3) Bounding info
			void				CG3DBoundInfo();
			void				SetBoundingInfo(const CG3D::SEGMENT& p_rSphere);
			const SEGMENT&		GetBounding() const							{	return m_Bounding;}
			const SEGMENT&		GetOriginalBounding() const					{	return m_Bounding_Original;}
...
protected:
	// 1) Original Bounding Info
			CG3D::SEGMENT		m_Bounding_Original;

	// 2) Transformed Bounding Info
			CG3D::SEGMENT		m_Bounding;
};
