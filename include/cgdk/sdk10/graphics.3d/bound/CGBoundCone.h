...
public:
	...
	// 3) Bounding info
			void				CG3DBoundInfo();
			void				SetBoundingInfo(const CONE& p_rSphere);
			const CONE&			GetBounding() const							{	return m_Bounding;}
			const CONE&			GetOriginalBounding() const					{	return m_Bounding_Original;}
...
protected:
	// 1) Original Bounding Info
			CG3D::CONE		m_Bounding_Original;

	// 2) Transformed Bounding Info
			CG3D::CONE		m_Bounding;
};
