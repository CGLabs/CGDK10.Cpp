...
//-----------------------------------------------------------------------------
//
// CChild_connector_control
//
// 1. CChild_connector_control란!
//    1) Socket의 상태정보를 표시하는 Dialog
//    2) DoModal()로 띄우는 것으로 작성하였다.
// 
// 
//-----------------------------------------------------------------------------
class CGNETSOCKETMFCCONTROL_EXT_CLASS CChild_connector_control : public CChildDialog
{
	DECLARE_DYNAMIC(CChild_connector_control)

// constructor/destructor)
public:
			CChild_connector_control(net::io::Iconnect_requestable* _pConnector=nullptr, CWnd* pParent=nullptr);   // standard constructor
	virtual ~CChild_connector_control();
...
protected:
			...
			uint16_t			m_port_bind;
			BOOL				m_bEnableControl;

			eSOCKET_STATE		m_state_pre;
};
