...
public:
	...
	using container_t = std::set<object_set_t>;
	using iterator_t = typename container_t::iterator;
	using const_iterator_t = typename container_t::const_iterator;

public:
			result_code			register_object(object_t* _pobject) { object_data_t tempObjectData; return register_object(_pobject, tempObjectData);}
...
result_code container::set<_TOBJECT, _TOBJECT_DATA, _TOBJECTBASE>::register_object(_TOBJECT* _pobject, const object_data_t& _object_data)
{
	...
	scoped_lock(this->m_lockable_container_enter)
	{
		...
		{
			// check) 이미 같은 Object가 존재하는지 확인한다.
			auto iter_find = m_container_object.find(temp_object_set);

			// check) 이미 붙어 있다면 끝낸다.
			RETURN_IF(iter_find != m_container_object.end(), eRESULT::ALEADY_EXIST);
		}

		// 1) on_object_registering()함수를 호출한다.
		...
		{
			// 3) 추가한다.
			auto result_insert = m_container_object.insert(temp_object_set);
			...
			catch(std::exception& e)
			{
				...
				// - object를 제거한다.
				m_container_object.erase(result_insert.first);
...
template <class _TOBJECT, class _TOBJECT_DATA, class _TOBJECTBASE>
bool container::set<_TOBJECT, _TOBJECT_DATA, _TOBJECTBASE>::unregister_object(_TOBJECT* _pobject) noexcept
{
	...
	scoped_lock(this->m_lockable_container)
	{
		...
		auto iter_find = m_container_object.find(object_set_t(_pobject));
...
template <class _TOBJECT, class _TOBJECT_DATA, class _TOBJECTBASE>
void container::set<_TOBJECT, _TOBJECT_DATA, _TOBJECTBASE>::unregister_object_all() noexcept
{
	// declare) 
...
template <class _TOBJECT, class _TOBJECT_DATA, class _TOBJECTBASE>
void container::set<_TOBJECT, _TOBJECT_DATA, _TOBJECTBASE>::capacity(size_t _max_count)
{
	scoped_lock(this->m_lockable_container)
	{
		// 1) 최대 Object수를 설정한다.
...
template <class _TOBJECT, class _TOBJECT_DATA, class _TOBJECTBASE>
object_ptr<_TOBJECT> container::set<_TOBJECT, _TOBJECT_DATA, _TOBJECTBASE>::find_object(const std::function<bool (_TOBJECT*)>& _pred) const noexcept
{
	scoped_lock(this->m_lockable_container)
	{
...
template <class _TOBJECT, class _TOBJECT_DATA, class _TOBJECTBASE>
bool container::set<_TOBJECT, _TOBJECT_DATA, _TOBJECTBASE>::for_each(const std::function<bool (_TOBJECT*)>& _function)
{
	scoped_lock(this->m_lockable_container)
	{
		for(auto& iter:m_container_object)
