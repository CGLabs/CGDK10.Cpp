...
class container::map :
	virtual public				Iserver_container<_TOBJECTBASE>
{
public:
	using key_t = _TKEY;
	...
	using container_t = std::map<key_t, object_set_t>;
	using iterator_t = typename container_t::iterator;
	using const_iterator_t = typename container_t::const_iterator;

public:
			result_code			register_object(const _TKEY& _key, object_t* _pobject) { object_data_t tempObjectData; return register_object(_key, _pobject, tempObjectData);}
	virtual	result_code			register_object(const _TKEY& _key, object_t* _pobject, const object_data_t& _object_data);
	virtual	bool				unregister_object(object_t* _pobject) noexcept override;
	virtual	bool				unregister_object(const _TKEY& _key) noexcept;
...
result_code container::map<_TOBJECT, _TKEY, _TOBJECT_DATA, _TOBJECTBASE>::register_object(const _TKEY& _key, _TOBJECT* _pobject, const object_data_t& _object_data)
{
	...
	scoped_lock(this->m_lockable_container_enter)
	{
		...
		{
			// 1) 이미 같은 Object가 존재하는지 확인한다.
			auto iter_find = m_container_object.find(_key);

			// check) 이미 붙어 있다면 끝낸다.
			RETURN_IF(iter_find != m_container_object.end(), eRESULT::NOT_EXIST);
		}
		...
		{
			// 3) 추가한다.
			auto result_insert = m_container_object.insert(std::make_pair(_key, temp_object_set));
...
template <class _TOBJECT, class _TKEY, class _TOBJECT_DATA, class _TOBJECTBASE>
bool container::map<_TOBJECT, _TKEY, _TOBJECT_DATA, _TOBJECTBASE>::unregister_object(_TOBJECT* _pobject) noexcept
{
	// check) _pobject가 nullptr이여서는 안됀다.
	CGASSERT_ERROR(_pobject != nullptr);

	// check) _pobject가 nullptr이여서는 안됀다.
	RETURN_IF(_pobject == nullptr, false);
	...
	scoped_lock(this->m_lockable_container)
	{
		...
		for(auto iter_end= m_container_object.end(); iter_find !=iter_end; ++iter_find)
		{
			BREAK_IF(iter_find->second.pobject == _pobject);
		}
		...
		// 3) on_object_unregistered()함수를 호출한다.
		try
		{
...
template <class _TOBJECT, class _TKEY, class _TOBJECT_DATA, class _TOBJECTBASE>
bool container::map<_TOBJECT, _TKEY, _TOBJECT_DATA, _TOBJECTBASE>::unregister_object(const _TKEY& _key) noexcept
{
	scoped_lock(this->m_lockable_container)
	{
		// declare) 
		...
		try
		{
			result = on_object_unregistering(iter_find->second.pobject);
		}
		// exception) 
		catch(std::exception& /*_e*/)
		{
			LOG_FATAL << "(excp) occure exception in 'on_object_unregistering' function on object"sv;
		}
		...
		try
		{
			on_object_unregistered(iter_find->second.pobject, result);
		}
		// exception) 
		catch(std::exception& /*_e*/)
		{
			LOG_FATAL << "(excp) occure exception in 'on_object_unregistered' function on object"sv;
		}
	}
...
template <class _TOBJECT, class _TKEY, class _TOBJECT_DATA, class _TOBJECTBASE>
void container::map<_TOBJECT, _TKEY, _TOBJECT_DATA, _TOBJECTBASE>::unregister_object_all() noexcept
{
	// declare) 
	...
	{
		unregister_object(iter.ocid);
	}
}

template <class _TOBJECT, class _TKEY, class _TOBJECT_DATA, class _TOBJECTBASE>
void container::map<_TOBJECT, _TKEY, _TOBJECT_DATA, _TOBJECTBASE>::capacity(size_t _max_count)
{
	scoped_lock(this->m_lockable_container)
	{
		// 1) 최대 Object수를 설정한다.
...
template <class _TOBJECT, class _TKEY, class _TOBJECT_DATA, class _TOBJECTBASE>
object_ptr<_TOBJECT> container::map<_TOBJECT, _TKEY, _TOBJECT_DATA, _TOBJECTBASE>::find_object(const std::function<bool (_TOBJECT*)>& _pred) const noexcept
{
	scoped_lock(this->m_lockable_container)
	{
	...
	// return) 찾지 못했다..
	return	object_ptr<_TOBJECT>();
}

template <class _TOBJECT, class _TKEY, class _TOBJECT_DATA, class _TOBJECTBASE>
bool container::map<_TOBJECT, _TKEY, _TOBJECT_DATA, _TOBJECTBASE>::for_each(const std::function<bool (_TOBJECT*)>& _function)
{
	scoped_lock(this->m_lockable_container)
	{
		for(auto& iter:m_container_object)
