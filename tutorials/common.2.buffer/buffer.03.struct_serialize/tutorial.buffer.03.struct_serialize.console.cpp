...
int main()
{
	...
	// ------------------------------------------------------------------
	{
		...
		// - bar의 size_값을 출력한다. 
		std::cout << "bar.size_: "sv << bar.size() << " bytes\n"sv;
		...
		// - foo를 출력한다.
		std::cout << "a: "sv << s1.a << '\n';
		std::cout << "b: "sv << static_cast<int32_t>(s1.b) << '\n';
		std::cout << "c: "sv << s1.c << '\n';
		std::cout << "d: "sv << s1.d << '\n';
		std::cout << '\n';
	}
	...
	// ------------------------------------------------------------------
	{
		struct goo
		{
			ENABLE_STRUCT_SERIALIZABLE	// {!} 이걸 넣어준다! 
		...
		// (*) get_size_of<T>(...)로 직렬화시 데이터 크기를 얻는다.
		const auto size = get_size_of(s1);

		std::cout << "sizeof s1:"sv << sizeof(s1) << " bytes  get_size_of s1: "sv << size << " bytes\n"sv;

		auto bar = alloc_shared_buffer(size);

		// (*) buf에 DATA형 변수 s1을 append한다.
		bar.append<goo>(s1);

		std::cout << "bar.size_: "sv << bar.size() << " bytes\n"sv;

		// (*) goo형을 extract해서 읽어낸다.
		...
		std::cout << "c: "sv << v1.c << '\n';
		std::cout << "d: "sv << v1.d << '\n';
		std::cout << "e: "sv; for(auto iter: v1.e) std::cout << iter << " "; std::cout << '\n';
		std::cout << '\n' << '\n';
	}
	...
	//
	//    'ENABLE_STRUCT_SERIALIZABLE'를 사용하면 make_shared_buffer를 사용해
	...
	// ------------------------------------------------------------------
	{
		...
		// - bar의 size_값을 출력한다. 
		std::cout << "bar.size: "sv << bar.size() << " bytes\n"sv;
		...
		std::cout << "c: "sv << v1.c << '\n';
		std::cout << "d: "sv << v1.d << '\n';
		std::cout << "e: "sv; for (auto iter : v1.e) std::cout << iter << " "sv;
		std::cout << '\n' << '\n';
	}
